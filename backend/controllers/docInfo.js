...
const getDocInfoByProviderId = (req, res) => {
  ...
  pool
    .query(
      `SELECT * FROM docInfo INNER JOIN specialty ON docInfo.specialty=specialty.specialty_id WHERE provider_id = $1`,
      [provider_id]
    )
    .then((result) => {
      res.status(200).json({
...
const deleteDocInfoByProviderId = (req, res) => {
  const provider_id = req.token.userId;
  pool
    .query(`delete from docInfo where provider_id = $1 RETURNING *`, [
      provider_id,
    ])
    .then((result) => {
