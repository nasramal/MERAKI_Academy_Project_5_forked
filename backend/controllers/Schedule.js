...
const getNotBookedScheduleByProviderId = (req, res) => {
  ...
  pool
    .query(
      `SELECT * FROM Schedule WHERE provider_id = $1 AND booked = false AND is_deleted=0`,
      [provider_id]
    )
    .then((result) => {
      res.status(200).json({
...
const updatedScheduleByID = (req, res) => {
  ...
  pool
    .query(
      `UPDATE schedule SET date = COALESCE($1,date), timeFrom = COALESCE($2, timeFrom), timeTo = COALESCE($3, timeTo) , booked = COALESCE($4,booked) WHERE schedule_id=$5 AND is_deleted = 0  RETURNING *`,
...
const deleteScheduleByProviderId = (req, res) => {
  ...
  pool
    .query(
      `UPDATE schedule SET is_deleted=1 WHERE schedule_id=$1 AND is_deleted = 0  RETURNING *`,
      [schedule_id]
    )
    .then((result) => {
      res.status(200).json({
