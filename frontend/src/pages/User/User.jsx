...
const User = () => {
    ...
    const getHistoryByUserId = async (userId) => {
        try {
            const result = await axios.get(`http://localhost:5000/history/gethistory`, {
                headers: {
                    Authorization: `Bearer ${token}`,
                },
            });
            ...
            if (result.data.success) {
                const updatedUsers = users.map(user =>
                    user.users_id === userId ? { ...user, history: result.data.history[0] } : user
                );
            ...
            {selectedUser && (
                <div>
                    ...
                    <button className="commentBtn" onClick={() => {
                        ...
                        {selectedUser.history ? "Update Medical History" : "Add Medical History"}
                    </button>
                </div>
            )}
